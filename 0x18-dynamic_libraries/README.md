...
## Details
      By Julien Barbier          Weight: 1              Project over - took place from 01-10-2022 to 01-11-2022          - you're done with 117% of tasks.                Manual QA review was done on 01-18-2022.              An auto review will be launched at the deadline      #### In a nutshell…
* Manual QA review:          0.0/18 mandatory      
* Auto QA review:          26.0/31 mandatory            &            10.0/20 optional      
* Altogether:         79.59%* Mandatory: 53.06%
* Optional: 50.0%
*               Calculation:                   53.06%                    + (53.06% * 50.0%)               == 79.59%
...
### 0. A library is not a luxury but one of the necessities of life
          mandatory         Progress vs Score           Score: 100.00% (Checks completed: 100.00%)         Task Body Create the dynamic library libdynamic.so containing all the functions listed below:
```bash
int _putchar(char c);
int _islower(int c);
int _isalpha(int c);
...
### 1. Without libraries what have we? We have no past and no future
          mandatory         Progress vs Score           Score: 50.00% (Checks completed: 50.00%)         Task Body Create a script that creates a dynamic library called   ` liball.so `   from all the   ` .c `   files that are in the current directory.
```bash
...
### 2. Either write something worth reading or do something worth writing
          mandatory         Progress vs Score           Score: 0.00% (Checks completed: 0.00%)         Task Body Write a blog post describing the differences between static and dynamic libraries. It should cover:
...
### 3. Let's call C functions from Python
          #advanced         Progress vs Score           Score: 100.00% (Checks completed: 100.00%)         Task Body I know, you’re missing C when coding in Python. So let’s fix that!
 [!brokenimg](https://images4.alphacoders.com/118/thumb-1920-11826.jpg) Create a dynamic library that contains C functions that can be called from Python. See example for more detail.
```bash
...
### 4. Code injection: Win the Giga Millions!
          #advanced         Progress vs Score           Score: 0.00% (Checks completed: 0.00%)         Task Body  ![](http://4.bp.blogspot.com/-9rqm8Pg1apY/UZHoC79xXtI/AAAAAAAACOs/0Fp4A4ipUqM/s320/tumblr_mlzp3qgHss1s5xo13o3_r1_1280.jpg) 
...
```
* You can’t modify the program  ` gm `  itself as Master Sysadmin Sylvain (MSS) always checks its [MD5](https://intranet.hbtn.io/rltoken/lDmQCzyC63ui2Q0uldZbPA) 
 before running it
